...
$("#carousel-testimonials-v2").owlCarousel({
    items: 2,
...
$('#btn-subscribe').on("click", function (e) {
    let value = $("#input-subscribe").val();
    let uriBase = window.location.protocol + "//" + window.location.host;
    ...
    if (value == "") {
        message = "Debe agregar su correo electrónico";
    }

    if (message == "") {
        $.ajax({
            url: `${uriBase}/public/subscribe/register/user`,
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            data: { 'user_email': value, 'medio': 'GlobalGrowth' },
            method: "POST",
            success: function (response) {
                console.info(response);
                $("#subcription-msg").html(response.message);
                $("#input-subscribe").val("");
            },
