                    ...
                    <div class="col-12 col-md-7 mt-4 mt-md-0 list">
                        <form id="form_maintenance" class="needs-validation" novalidate>
                            @foreach ($workshop as $key => $item)
                                <div class="content" id="content_{{ $item->id }}">
                                    <input type="hidden" class="txtId" value="{{ $item->id }}" name="id">

                                    <input type="hidden" class="txtLat" value="{{ $item->lat }}" name="lat">
                                    <input type="hidden" class="txtLng" value="{{ $item->lng }}" name="lng">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-6 text-right">
                                                <button class="btn btn-danger btn-delete" type="button"
                                                    itemId='{{ $item->id }}'><i class="fas fa-trash-alt"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control txtDepartament" name="departament"
                                            value="{{ $item->departament }}" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="phone">Telefono</label>
                                        <input type="text" class="form-control txtPhone" name="phone"
                                        placeholder="Ejemplo +51 987654321" value="{{ $item->phone }}" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="link_cita">Enlace de Cita</label>
                                        <input type="text" class="form-control txtLink_cita" name="link_cita"
                                            value="{{ $item->link_cita }}" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="map_link">Insertar Ubicación</label>
                                        <div class="input-group mb-3">
                                            <input type="text" class="form-control txtMap_link"
                                                value="{{ $item->map_link }}" id="txtDirection_{{ $item->id }}"
                                                aria-describedby="btn-modal-map-{{ $item->id }}" readonly
                                                name="direction{{ $item->id }}" required>
                                            <div class="input-group-append">
                                                <button class="btn btn-outline-secondary btn-open-map"
                                                    item_id="{{ $item->id }}" type="button"
                                                    content="txtDirection_{{ $item->id }}" lng=""
                                                    lat="{{ $item->lat }}" name="map_link"
                                                    id="btn-modal-map-{{ $item->id }}"><i
                            ...
                            <div class="item-image mt-4">
                                <div class="content-file-img mb-4 d-flex justify-content-between align-items-center">
                                    ...
                                    <button class="btn btn-outline-secondary btn-file btn-sm"
                                        img-content="img-{{ $item->id }}"
                                        input-file="fileInputImage_{{ $item->id }}">
                                        <i class="fas fa-plus"></i> Cargar Imagen
                                    </button>
                                    <input type="file" class="d-none input-file" id="fileInputImage_{{ $item->id }}"
                                        lang="es" item_id="{{ $item->id }}" accept=".jpg,.png,.jpeg,.svg">
                                </div>
                                <div class="pl-5 pr-5">
                                    <img src="{{ $item->image }}" alt="" class="img-fluid shadow "
                                        id="img-{{ $item->id }}">
                                </div>
